...
export async function initSpeakerRetrievalChain() {
  ...
  const store = await Neo4jVectorStore.fromExistingGraph(embeddings, {
    ...
    nodeLabel: "Speaker",
    textNodeProperties: ["name", "bio"],
    indexName: "speaker_embeddings_openai",
    embeddingNodeProperty: "embedding",
    retrievalQuery: `
    RETURN node.name + ': '+ node.bio AS text,
    node { .name, .company, .x_handle,
      talks: [ (t)-[:GIVEN_BY]->(node) | t {
          .title, .description, .url, .time
      } ]
    } AS metadata
`,
  });
